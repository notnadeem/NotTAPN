...
public:
  ...
  convert(const TAPN::TimedArcPetriNet &tapn,
    ...
    std::unordered_map<const TAPN::TimedTransition *, CudaTimedTransition *>
        transitionMap;
    std::unordered_map<const TAPN::TimedInputArc *, CudaTimedInputArc *>
        inputArcMap;
    std::unordered_map<const TAPN::TransportArc *, CudaTimedTransportArc *>
        transportArcMap;
    std::unordered_map<const TAPN::InhibitorArc *, CudaTimedInhibitorArc *>
        inhibitorArcMap;
    std::unordered_map<const TAPN::OutputArc *, CudaTimedOutputArc *>
        outputArcMap;
    ...
    auto transitions =
        new CudaTimedTransition *[tapn.getTransitions().size()];
    for (size_t i = 0; i < tapn.getTransitions().size(); i++) {
      auto originalTransition = tapn.getTransitions()[i];
      transitions[i] =
          new CudaTimedTransition(convertTransition(*originalTransition));
    ...
    for (size_t i = 0; i < tapn.getInputArcs().size(); i++) {
      ...
      inputArcs[i] = new CudaTimedInputArc(
    ...
    for (size_t i = 0; i < tapn.getPlaces().size(); i++) {
      ...
      auto inputArcs =
          new CudaTimedInputArc *[originalPlace->getInputArcs().size()];
      ...
      auto transportArcs = new CudaTimedTransportArc
      ...
      cudaPlace->transportArcsLength =
          originalPlace->getTransportArcs().size();

      auto prodTransportArcs = new CudaTimedTransportArc
      ...
      cudaPlace->prodTransportArcsLength =
          originalPlace->getProdTransportArcs().size();

      auto inhibitorArcs = new CudaTimedInhibitorArc
      ...
      cudaPlace->inhibitorArcsLength =
          originalPlace->getInhibitorArcs().size();

      auto outputArcs =
          new CudaTimedOutputArc *[originalPlace->getOutputArcs().size()];
    ...
    for (size_t i = 0; i < tapn.getTransitions().size(); i++) {
      ...
      auto inputArcs =
          new CudaTimedInputArc *[originalTransition->getPreset().size()];
      ...
      auto outputArcs =
          new CudaTimedOutputArc *[originalTransition->getPostset().size()];
      ...
      cudaTransition->transportArcsLength =
          originalTransition->getTransportArcs().size();
      ...
      cudaTransition->inhibitorArcsLength =
          originalTransition->getInhibitorArcs().size();
    }
...
private:
  ...
  static CudaSMC::Distribution
  convertDistribution(const SMC::Distribution &distribution) {
    CudaSMC::Distribution cudaDistribution;
    cudaDistribution.type =
        static_cast<CudaSMC::DistributionType>(distribution.type);
    cudaDistribution.parameters =
        *reinterpret_cast<const CudaSMC::DistributionParameters *>(
