...
public:
  static std::pair<SimpleTimedArcPetriNet, SimpleRealMarking *> *convert(const TAPN::TimedArcPetriNet &tapn,
                                                                         DiscreteVerification::RealMarking &marking) {
    std::unordered_map<const TAPN::TimedPlace *, SimpleTimedPlace *> placeMap;
    std::unordered_map<const TAPN::TimedTransition *, SimpleTimedTransition *> transitionMap;
    std::unordered_map<const TAPN::TimedInputArc *, SimpleTimedInputArc *> inputArcMap;
    std::unordered_map<const TAPN::TransportArc *, SimpleTimedTransportArc *> transportArcMap;
    std::unordered_map<const TAPN::InhibitorArc *, SimpleTimedInhibitorArc *> inhibitorArcMap;
    std::unordered_map<const TAPN::OutputArc *, SimpleTimedOutputArc *> outputArcMap;
    ...
    auto transitions = new SimpleTimedTransition *[tapn.getTransitions().size()];
    for (size_t i = 0; i < tapn.getTransitions().size(); i++) {
      auto originalTransition = tapn.getTransitions()[i];
      transitions[i] = new SimpleTimedTransition(convertTransition(*originalTransition));
    ...
    for (size_t i = 0; i < tapn.getInputArcs().size(); i++) {
      ...
      inputArcs[i] = new SimpleTimedInputArc(convertInputArc(*originalArc, placeMap, transitionMap));
    ...
    for (size_t i = 0; i < tapn.getOutputArcs().size(); i++) {
      ...
      outputArcs[i] = new SimpleTimedOutputArc(convertOutputArc(*originalArc, placeMap, transitionMap));
      outputArcMap[originalArc] = outputArcs[i];
    }

    auto transportArcs = new SimpleTimedTransportArc *[tapn.getTransportArcs().size()];
    for (size_t i = 0; i < tapn.getTransportArcs().size(); i++) {
      auto originalArc = tapn.getTransportArcs()[i];
      transportArcs[i] = new SimpleTimedTransportArc(convertTransportArc(*originalArc, placeMap, transitionMap));
      transportArcMap[originalArc] = transportArcs[i];
    }

    auto inhibitorArcs = new SimpleTimedInhibitorArc *[tapn.getInhibitorArcs().size()];
    for (size_t i = 0; i < tapn.getInhibitorArcs().size(); i++) {
      auto originalArc = tapn.getInhibitorArcs()[i];
      inhibitorArcs[i] = new SimpleTimedInhibitorArc(convertInhibitorArc(*originalArc, placeMap, transitionMap));
    ...
    for (size_t i = 0; i < tapn.getPlaces().size(); i++) {
      ...
      auto inputArcs = new SimpleTimedInputArc *[originalPlace->getInputArcs().size()];
      ...
      auto transportArcs = new SimpleTimedTransportArc *[originalPlace->getTransportArcs().size()];
      ...
      simplePlace->transportArcsLength = originalPlace->getTransportArcs().size();

      auto prodTransportArcs = new SimpleTimedTransportArc *[originalPlace->getProdTransportArcs().size()];
      ...
      simplePlace->prodTransportArcsLength = originalPlace->getProdTransportArcs().size();

      auto inhibitorArcs = new SimpleTimedInhibitorArc *[originalPlace->getInhibitorArcs().size()];
      ...
      simplePlace->inhibitorArcsLength = originalPlace->getInhibitorArcs().size();

      auto outputArcs = new SimpleTimedOutputArc *[originalPlace->getOutputArcs().size()];
    ...
    for (size_t i = 0; i < tapn.getTransitions().size(); i++) {
      ...
      auto inputArcs = new SimpleTimedInputArc *[originalTransition->getPreset().size()];
      ...
      auto outputArcs = new SimpleTimedOutputArc *[originalTransition->getPostset().size()];
      ...
      auto transportArcs = new SimpleTimedTransportArc *[originalTransition->getTransportArcs().size()];
      ...
      simpleTransition->transportArcsLength = originalTransition->getTransportArcs().size();

      auto inhibitorArcs = new SimpleTimedInhibitorArc *[originalTransition->getInhibitorArcs().size()];
      ...
      simpleTransition->inhibitorArcsLength = originalTransition->getInhibitorArcs().size();
    }
    ...
    for (size_t i = 0; i < placeLength; i++) {
      ...
      for (size_t j = 0; j < tokenLength; j++) {
        ...
        srm->places[i]->tokens->add(new SimpleRealToken{realPlace.tokens[j].getAge(), realPlace.tokens[j].getCount()});
      }
...
private:
  ...
  static SimpleTimedTransition convertTransition(const TAPN::TimedTransition &transition) {
    ...
    simpleTransition.distribution = convertDistribution(transition.getDistribution());
    simpleTransition._weight = transition.getWeight();
    simpleTransition._firingMode = convertFiringMode(transition.getFiringMode());
    return simpleTransition;
  }

  static SimpleTimedInputArc
  convertInputArc(const TAPN::TimedInputArc &arc,
                  const std::unordered_map<const TAPN::TimedPlace *, SimpleTimedPlace *> &placeMap,
                  const std::unordered_map<const TAPN::TimedTransition *, SimpleTimedTransition *> &transitionMap) {
  ...
  static SimpleTimedTransportArc
  ...
  static SimpleTimedInhibitorArc
  ...
  static SimpleTimeInterval convertTimeInterval(const TAPN::TimeInterval &interval) {
    return {interval.isLowerBoundStrict(), interval.getLowerBound(), interval.getUpperBound(),
            interval.isUpperBoundStrict()};
  }

  static SimpleTimeInvariant convertTimeInvariant(const TAPN::TimeInvariant &invariant) {
    // print the invariant
    return {invariant.isBoundStrict(), invariant.getBound()};
  }

  static SimpleSMC::Distribution convertDistribution(const SMC::Distribution &distribution) {
    SimpleSMC::Distribution simpleDistribution;
    simpleDistribution.type = static_cast<SimpleSMC::DistributionType>(distribution.type);
    simpleDistribution.parameters =
        *reinterpret_cast<const SimpleSMC::DistributionParameters *>(&distribution.parameters);
