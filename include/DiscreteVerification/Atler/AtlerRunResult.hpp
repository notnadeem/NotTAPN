...
struct AtlerRunResult {
  ...
  SimpleDynamicArray<SimpleDynamicArray<Util::SimpleInterval>> *transitionIntervals;
  ...
  AtlerRunResult(SimpleTimedArcPetriNet tapn, SimpleRealMarking *srm, const unsigned int numericPrecision = 0)
      : tapn(tapn), parent(srm), numericPrecision(numericPrecision) {
    std::random_device rd;
    rng = std::ranlux48(rd());
    transitionIntervals = new SimpleDynamicArray<SimpleDynamicArray<Util::SimpleInterval>>(tapn.transitionsLength);
  }
  ...
  AtlerRunResult(const AtlerRunResult &other) {
    ...
    transitionIntervals =
        new SimpleDynamicArray<SimpleDynamicArray<Util::SimpleInterval>>(other.transitionIntervals->size);
    for (size_t i = 0; i < other.transitionIntervals->size; i++) {
      auto intervals = SimpleDynamicArray<Util::SimpleInterval>(other.transitionIntervals->get(i).size);
  ...
  void prepare() {
    ...
    auto invIntervals = SimpleDynamicArray<Util::SimpleInterval>(tapn.transitionsLength);
    ...
    for (size_t i = 0; i < tapn.transitionsLength; i++) {
      ...
      if (transition->getPresetSize() == 0 && transition->inhibitorArcs == 0) {
        ...
        transitionIntervals->add(Util::setIntersection(firingDates, invIntervals));
  ...
  void refreshTransitionsIntervals() {
    ...
    for (size_t i = 0; i < tapn.transitionsLength; i++) {
      ...
      if (transition->getPresetSize() == 0 && transition->inhibitorArcsLength == 0) {
        ...
        SimpleDynamicArray<Util::SimpleInterval> arco = Util::setIntersection(firingDates, invIntervals);
  ...
  SimpleDynamicArray<Util::SimpleInterval> transitionFiringDates(const SimpleTimedTransition &transition) {
    ...
    firingIntervals.add(Util::SimpleInterval(0, std::numeric_limits<double>::infinity()));
    ...
    for (size_t i = 0; i < transition.transportArcsLength; i++) {
      ...
      auto arcFire = arcFiringDates(arcInterval, transport->weight, *place->tokens);
...
struct AtlerRunResult2 {
  ...
  AtlerRunResult2(SimpleTimedArcPetriNet stapn, SimpleRealMarking srm, const unsigned int numericPrecision = 0)
    ...
    transitionIntervals = new SimpleDynamicArray<SimpleDynamicArray<Util::SimpleInterval> *>(stapn.transitionsLength);
  ...
  void prepare() {
    ...
    std::cout << "stapn transitions length: " << stapn.transitionsLength << std::endl;
    std::cout << "transition intervals length: " << transitionIntervals->size << std::endl;
    reset();
  }
  ...
  void reset() {
    ...
    for (size_t i = 0; i < datesSampled->size; i++) {
      auto *intervals = transitionIntervals->get(i);
      if (!intervals->empty() && intervals->get(0).lower() == 0) {
        const SimpleSMC::Distribution distribution = stapn.transitions[i]->distribution;
  ...
  bool fire(SimpleTimedTransition *transition) {
    ...
    for (size_t i = 0; i < transition->transportArcsLength; i++) {
      ...
      for (size_t j = 0; j < consumed.size; j++) {
        ...
        toCreate.add(std::make_pair(&(transport->destination), SimpleRealToken{con.age, con.count}));
      }
    }
