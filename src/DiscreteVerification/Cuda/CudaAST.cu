...
__host__ __device__ CudaExpression::CudaExpression() : type(UNKNOWN_EXPRESSION), eval(0), notExpr(nullptr) {}

__device__ CudaExpression::~CudaExpression() {
...
__device__ void CudaExpression::accept(CudaVisitor &visitor, Result &context) {
  switch (type) {
  case NOT_EXPRESSION:
...
__host__ __device__ NotExpression::NotExpression(CudaExpression *expr) : expr(expr) {}

__host__ __device__ NotExpression *NotExpression::clone() const {
  return new NotExpression(expr ? expr->clone() : nullptr);
}

__device__ void NotExpression::accept(CudaVisitor &visitor, Result &context) { visitor.visit(*this, context); }

// Definitions of DeadlockExpression methods
__host__ __device__ DeadlockExpression::DeadlockExpression() {}

__host__ __device__ DeadlockExpression *DeadlockExpression::clone() const { return new DeadlockExpression(); }

__device__ void DeadlockExpression::accept(CudaVisitor &visitor, Result &context) {
  visitor.visit(*this, context);
}

// Definitions of BoolExpression methods
__host__ __device__ BoolExpression::BoolExpression(bool value) : value(value) {}

__host__ __device__ BoolExpression *BoolExpression::clone() const { return new BoolExpression(value); }

__device__ void BoolExpression::accept(CudaVisitor &visitor, Result &context) {
  visitor.visit(*this, context);
}
...
__host__ __device__ inline void customSwap(ArithmeticExpression *&a, ArithmeticExpression *&b) {
  ArithmeticExpression *temp = a;
  a = b;
  b = temp;
}

__host__ __device__ inline int customStrcmp(const char* str1, const char* str2) {
...
__host__ __device__ AtomicProposition::AtomicProposition(ArithmeticExpression *l, const char *sop,
                                                         ArithmeticExpression *r)
    : left(l), right(r) {
  if (customStrcmp(sop, "=") == 0 || customStrcmp(sop, "==") == 0)
    op = EQ;
  else if (customStrcmp(sop, "!=") == 0)
    op = NE;
  else if (customStrcmp(sop, "<") == 0)
    op = LT;
  else if (customStrcmp(sop, "<=") == 0)
...
__host__ __device__ AtomicProposition::AtomicProposition(ArithmeticExpression *left, op_e op,
...
__device__ void AtomicProposition::accept(CudaVisitor &visitor, Result &context) {
  visitor.visit(*this, context);
}
...
__device__ void ArithmeticExpression::accept(CudaVisitor &visitor, Result &context) {
  switch (type) {
  case PLUS_EXPRESSION:
...
__device__ void PlusExpression::accept(CudaVisitor &visitor, Result &context) {
  visitor.visit(*this, context);
}
...
__device__ void SubtractExpression::accept(CudaVisitor &visitor, Result &context) {
  visitor.visit(*this, context);
}

// Definitions of MinusExpression methods
__host__ __device__ MinusExpression::MinusExpression(ArithmeticExpression *value) : value(value) {}

__host__ __device__ MinusExpression *MinusExpression::clone() const {
  return new MinusExpression(value ? value->clone() : nullptr);
}

__device__ void MinusExpression::accept(CudaVisitor &visitor, Result &context) {
  visitor.visit(*this, context);
}
...
__device__ void MultiplyExpression::accept(CudaVisitor &visitor, Result &context) {
  visitor.visit(*this, context);
}

// Definitions of NumberExpression methods
__host__ __device__ NumberExpression::NumberExpression(int value) : value(value) {}

__host__ __device__ NumberExpression *NumberExpression::clone() const { return new NumberExpression(*this); }

__device__ void NumberExpression::accept(CudaVisitor &visitor, Result &context) {
  visitor.visit(*this, context);
}

// Definitions of IdentifierExpression methods
__host__ __device__ IdentifierExpression::IdentifierExpression(int place) : place(place) {}

__host__ __device__ IdentifierExpression *IdentifierExpression::clone() const {
  return new IdentifierExpression(*this);
}

__device__ void IdentifierExpression::accept(CudaVisitor &visitor, Result &context) {
  visitor.visit(*this, context);
}
...
__host__ __device__ CudaQuery::~CudaQuery() { delete expr; }

__host__ __device__ CudaQuery *CudaQuery::clone() const { return new CudaQuery(*this); }

__device__ void CudaQuery::accept(CudaVisitor &visitor, Result &context) { visitor.visit(*this, context); }

__device__ CudaQuantifier CudaQuery::getQuantifier() const { return quantifier; }

__device__ const CudaExpression &CudaQuery::getConstChild() const { return *expr; }

__device__ CudaExpression *CudaQuery::getChild() { return expr; }

__device__ void CudaQuery::setChild(CudaExpression *expr) { this->expr = expr; }

 __device__ void CudaQuery::setQuantifier(CudaQuantifier q) { quantifier = q; }

__host__ __device__ bool CudaQuery::hasSMCQuantifier() const { return quantifier == PF || quantifier == PG; }
