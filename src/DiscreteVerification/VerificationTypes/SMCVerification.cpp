  ...
  for (int i = 0; i < n_threads; i++) {
    auto handle = new std::thread([this]() {
      ...
      while (continueExecution) {
        ...
        double runDuration =
            std::min(generator.getRunDelay(), (double)smcSettings.timeBound);
...
bool SMCVerification::run() {
  ...
  while (mustDoAnotherRun()) {
    ...
    double runDuration =
        std::min(runGenerator.getRunDelay(), (double)smcSettings.timeBound);
    ...
    stepDuration =
        std::chrono::duration_cast<std::chrono::milliseconds>(step2 - step1)
            .count();
    if (stepDuration >= STEP_MS) {
      step1 = step2;
      stepDuration =
          std::chrono::duration_cast<std::chrono::milliseconds>(step2 - start)
              .count();
      std::cout << ". Duration : " << stepDuration
                << "ms ; Runs executed : " << numberOfRuns << std::endl;
    }
  }
  auto stop = std::chrono::steady_clock::now();
  durationNs =
...
unsigned int SMCVerification::maxUsedTokens() { return maxTokensSeen; }
...
bool SMCVerification::reachedRunBound(SMCRunGenerator *generator) {
  if (generator == nullptr)
    generator = &runGenerator;
  return generator->getRunDelay() >= smcSettings.timeBound ||
         generator->getRunSteps() >= smcSettings.stepBound;
}

void SMCVerification::handleTrace(const bool runRes,
                                  SMCRunGenerator *generator) {
  if (generator == nullptr)
    generator = &runGenerator;
...
void SMCVerification::printHumanTrace(std::stack<RealMarking *> &stack,
...
void SMCVerification::printXMLTrace(std::stack<RealMarking *> &stack,
                                    const std::string &name,
  ...
  while (!stack.empty()) {
    if (isFirst) {
      ...
      RealMarking *marking = stack.top();
      if (marking->getPreviousDelay() > 0) {
        std::string str = printDouble(marking->getPreviousDelay(),
...
rapidxml::xml_node<> *
SMCVerification::createTransitionNode(RealMarking *old, RealMarking *current,
                                      rapidxml::xml_document<> &doc) {
  using namespace rapidxml;
...
void SMCVerification::createTransitionSubNodes(
...
rapidxml::xml_node<> *
SMCVerification::createTokenNode(rapidxml::xml_document<> &doc,
