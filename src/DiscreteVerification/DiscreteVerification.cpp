...
template <typename T>
void VerifyAndPrint(TAPN::TimedArcPetriNet &tapn, Verification<T> &verifier,
                    VerificationOptions &options, AST::Query *query);

void ComputeAndPrint(TAPN::TimedArcPetriNet &tapn, SMCVerification &verifier,
                     VerificationOptions &options, AST::Query *query);

void AtlerComputeAndPrint(TAPN::TimedArcPetriNet &tapn, AtlerProbabilityEstimation &verifier,
                        VerificationOptions &options, AST::Query *query);
...
int DiscreteVerification::run(TAPN::TimedArcPetriNet &tapn,
                              const std::vector<int> &initialPlacement,
  ...
  NonStrictMarking *initialMarking =
      new NonStrictMarking(tapn, initialPlacement);

  std::cout << "MC: " << tapn.getMaxConstant() << std::endl;
#if DEBUG
  std::cout << "Places: " << std::endl;
  for (TAPN::TimedPlace::Vector::const_iterator iter = tapn.getPlaces().begin();
       iter != tapn.getPlaces().end(); iter++) {
    std::cout << "Place " << (*iter)->getIndex() << " has category "
              << (*iter)->getType() << std::endl;
  }
#endif

  if (initialMarking->size() > options.getKBound() &&
      !query->hasSMCQuantifier()) {
    std::cout << "The specified k-bound is less than the number of tokens in "
                 "the initial markings.";
    return 1;
  }

  if (query->hasSMCQuantifier()) {
    std::cout << "SMC Verification (all irrelevant options will be ignored)"
              << std::endl;
  ...
  if (options.getWorkflowMode() != VerificationOptions::NOT_WORKFLOW) {
    if (options.getVerificationType() == VerificationOptions::TIMEDART) {
      std::cout << "Workflow analysis currently only supports discrete "
                   "exploration (i.e. not TimeDarts)."
                << std::endl;
      std::exit(1);
    }
    ...
    if (options.getWorkflowMode() == VerificationOptions::WORKFLOW_SOUNDNESS) {
      ...
      VerifyAndPrint(tapn, *verifier, options, query);
      ...
      if (options.getMemoryOptimization() ==
          VerificationOptions::NO_MEMORY_OPTIMIZATION) {
        verifier = new WorkflowStrongSoundnessReachability(
            tapn, *initialMarking, query, options,
            getWaitingList<NonStrictMarking *>(query, options));
      } else {
        verifier = new WorkflowStrongSoundnessPTrie(
            tapn, *initialMarking, query, options,
            getWaitingList<ptriepointer_t<MetaData *>>(query, options));
      }
      VerifyAndPrint(tapn, *verifier, options, query);
  ...
  if (containsDeadlock.value && options.getGCDLowerGuardsEnabled()) {
    std::cout << "Lowering constants by greatest common divisor is unsound for "
                 "queries containing the deadlock proposition"
              << std::endl;
    std::exit(1);
  } else if (containsDeadlock.value &&
             options.getTrace() == VerificationOptions::FASTEST_TRACE) {
    std::cout << "Fastest trace is not supported for queries containing the "
                 "deadlock proposition."
              << std::endl;
    std::exit(1);
  }

  if ((query->getQuantifier() == EG || query->getQuantifier() == AF) &&
      options.getGCDLowerGuardsEnabled()) {
    std::cout << "Lowering constants by greatest common divisor is unsound for "
                 "EG and AF queries"
              << std::endl;
    std::exit(1);
  ...
  if (query->getQuantifier() == CG || query->getQuantifier() == CF) {
    ...
    if (options.getVerificationType() != VerificationOptions::DISCRETE) {
      std::cout << "TimeDarts are not supported for game synthesis"
                << std::endl;
      std::exit(1);
    }
    if (options.getGCDLowerGuardsEnabled()) {
      std::cout << "Lowering by GCD is not supported for game synthesis"
                << std::endl;
      std::exit(1);
    }
    if (options.getSearchType() == VerificationOptions::MINDELAYFIRST) {
      std::cout
          << "Minimal delay search strategy is not supported for game synthesis"
          << std::endl;
      std::exit(1);
    }

    // Only needed if verifying normal CTL/LTL with game-algorithm.
    // Notice that violating k-bound produces different results than in normal
    // ctl semantics
    if (query->getQuantifier() == AST::EG ||
        query->getQuantifier() == AST::AF) {
      tapn.setAllControllable(true);
    } else if (query->getQuantifier() == AST::EF ||
               query->getQuantifier() == AST::AG) {
      tapn.setAllControllable(false);
      query->setChild(new NotExpression(query->getChild()));
    }

    SafetySynthesis synthesis =
        SafetySynthesis(tapn, *initialMarking, query, options);
    ...
    if (query->getQuantifier() == AST::EF || query->getQuantifier() == AST::AF)
      result = !result;

    std::cout << "Query is " << (result ? "satisfied" : "NOT satisfied") << "."
              << std::endl;
    ...
    if (options.isBenchmarkMode()) {
      ...
      // ProbabilityEstimation estimator(tapn, marking, smcQuery, options);
      // ComputeAndPrint(tapn, estimator, options, query);
      AtlerProbabilityEstimation estimator(tapn, marking, smcQuery, options);
      AtlerComputeAndPrint(tapn, estimator, options, query);
    ...
    if (options.getMemoryOptimization() == VerificationOptions::PTRIE) {
      ...
      WaitingList<ptriepointer_t<MetaData *>> *strategy =
          getWaitingList<ptriepointer_t<MetaData *>>(query, options);
      if (query->getQuantifier() == EG || query->getQuantifier() == AF) {
        LivenessSearchPTrie verifier = LivenessSearchPTrie(
            tapn, *initialMarking, query, options, strategy);
        VerifyAndPrint(tapn, verifier, options, query);
      } else if (query->getQuantifier() == EF || query->getQuantifier() == AG) {
        if (options.getPartialOrderReduction()) {
          auto verifier = ReachabilitySearchPTrie<ReducingGenerator>(
              tapn, *initialMarking, query, options, strategy);
          VerifyAndPrint(tapn, verifier, options, query);
        } else {
          auto verifier = ReachabilitySearchPTrie<Generator>(
              tapn, *initialMarking, query, options, strategy);
          VerifyAndPrint(tapn, verifier, options, query);
        }
      }
      ...
      WaitingList<NonStrictMarking *> *strategy =
          getWaitingList<NonStrictMarking *>(query, options);
      if (query->getQuantifier() == EG || query->getQuantifier() == AF) {
        LivenessSearch verifier =
            LivenessSearch(tapn, *initialMarking, query, options, strategy);
        VerifyAndPrint(tapn, verifier, options, query);
      } else if (query->getQuantifier() == EF || query->getQuantifier() == AG) {
        if (options.getPartialOrderReduction()) {
          auto verifier = ReachabilitySearch<ReducingGenerator>(
              tapn, *initialMarking, query, options, strategy);
          VerifyAndPrint(tapn, verifier, options, query);
        } else {
          auto verifier = ReachabilitySearch<Generator>(
              tapn, *initialMarking, query, options, strategy);
          VerifyAndPrint(tapn, verifier, options, query);
        }
      }
    ...
    if (query->getQuantifier() == EG || query->getQuantifier() == AF) {
      if (containsDeadlock.value) {
        std::cout << "The combination of TimeDarts, Deadlock proposition and "
                     "EG or AF queries is currently not supported"
                  << std::endl;
        std::exit(1);
      }
      if (options.getMemoryOptimization() == VerificationOptions::PTRIE) {
        WaitingList<std::pair<WaitingDart *, ptriepointer_t<LivenessDart *>>>
            *strategy = getWaitingList<
                std::pair<WaitingDart *, ptriepointer_t<LivenessDart *>>>(
                query, options);
        TimeDartLivenessPData verifier = TimeDartLivenessPData(
            tapn, *initialMarking, query, options, strategy);
        VerifyAndPrint(tapn, verifier, options, query);
        delete strategy;
      } else {
        WaitingList<WaitingDart *> *strategy =
            getWaitingList<WaitingDart *>(query, options);
        TimeDartLiveness verifier =
            TimeDartLiveness(tapn, *initialMarking, query, options, strategy);
        VerifyAndPrint(tapn, verifier, options, query);
        delete strategy;
      }
    } else if (query->getQuantifier() == EF || query->getQuantifier() == AG) {
      ...
      if (options.getMemoryOptimization() == VerificationOptions::PTRIE) {
        WaitingList<TimeDartEncodingPointer> *strategy =
            getWaitingList<TimeDartEncodingPointer>(query, options);
        TimeDartReachabilitySearchPData verifier =
            TimeDartReachabilitySearchPData(tapn, *initialMarking, query,
                                            options, strategy);
        VerifyAndPrint(tapn, verifier, options, query);
        delete strategy;
      } else {
        WaitingList<TimeDartBase *> *strategy =
            getWaitingList<TimeDartBase *>(query, options);
        TimeDartReachabilitySearch verifier = TimeDartReachabilitySearch(
            tapn, *initialMarking, query, options, strategy);
        VerifyAndPrint(tapn, verifier, options, query);
        delete strategy;
      }
...
void VerifyAndPrint(TAPN::TimedArcPetriNet &tapn, Verification<T> &verifier,
                    VerificationOptions &options, AST::Query *query) {
  bool result = (!options.isWorkflow() &&
                 (query->getQuantifier() == AG || query->getQuantifier() == AF))
                    ? !verifier.run()
                    : verifier.run();

  if (options.getGCDLowerGuardsEnabled()) {
    std::cout << "Lowering all guards by greatest common divisor: "
              << tapn.getGCD() << std::endl;
  }
  std::cout << std::endl;
  ...
  if (options.getTrace() != VerificationOptions::NO_TRACE) {
    if ((query->getQuantifier() == EF && result) ||
        (query->getQuantifier() == AG && !result) ||
        (query->getQuantifier() == EG && result) ||
        (query->getQuantifier() == AF && !result) || (options.isWorkflow())) {
      verifier.getTrace();
    } else {
      std::cout << "A trace could not be generated due to the query result"
                << std::endl;
    }
  }
}

void ComputeAndPrint(TAPN::TimedArcPetriNet &tapn, SMCVerification &estimator,
                     VerificationOptions &options, AST::Query *query) {
...
// Create a AltComputeAndPrint function that takes a probability estimator
void AtlerComputeAndPrint(TAPN::TimedArcPetriNet &tapn, AtlerProbabilityEstimation &estimator,
                     VerificationOptions &options, AST::Query *query) {
