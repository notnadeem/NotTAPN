...
int DiscreteVerification::run(TAPN::TimedArcPetriNet &tapn, const std::vector<int> &initialPlacement, AST::Query *query,
  ...
  for (TAPN::TimedPlace::Vector::const_iterator iter = tapn.getPlaces().begin(); iter != tapn.getPlaces().end();
       iter++) {
  ...
  if (query->hasSMCQuantifier()) {
    ...
    if (!options.getQueryFile().empty()) std::cout << "Query file is: " << options.getQueryFile() << std::endl;
  } else {
    std::cout << options;
  }
  ...
  if (query->getQuantifier() == CG || query->getQuantifier() == CF) {
    ...
    SafetySynthesis synthesis = SafetySynthesis(tapn, *initialMarking, query, options);
    ...
    if (query->getQuantifier() == EG || query->getQuantifier() == AF) {
      ...
      if (options.getMemoryOptimization() == VerificationOptions::PTRIE) {
        WaitingList<std::pair<WaitingDart *, ptriepointer_t<LivenessDart *>>> *strategy =
            getWaitingList<std::pair<WaitingDart *, ptriepointer_t<LivenessDart *>>>(query, options);
      ...
      if (options.getMemoryOptimization() == VerificationOptions::PTRIE) {
        ...
        TimeDartReachabilitySearchPData verifier =
            TimeDartReachabilitySearchPData(tapn, *initialMarking, query, options, strategy);
        VerifyAndPrint(tapn, verifier, options, query);
        delete strategy;
      } else {
        WaitingList<TimeDartBase *> *strategy = getWaitingList<TimeDartBase *>(query, options);
        TimeDartReachabilitySearch verifier =
            TimeDartReachabilitySearch(tapn, *initialMarking, query, options, strategy);
        VerifyAndPrint(tapn, verifier, options, query);
        delete strategy;
      }
    }
