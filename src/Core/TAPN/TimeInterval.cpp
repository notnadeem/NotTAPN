...
TimeInterval
TimeInterval::createFor(const std::string &interval,
  ...
  std::string strUpperBound =
      splitVector[1].substr(0, splitVector[1].size() - 1);
  trim(strUpperBound);
...
void TimeInterval::print(std::ostream &out) const {
  ...
  std::string strUpperBound = upperBound == std::numeric_limits<int>::max()
                                  ? "inf"
                                  : std::to_string(upperBound);

  out << leftParenthesis << strLowerBound << "," << strUpperBound
      << rightParenthesis;
}
